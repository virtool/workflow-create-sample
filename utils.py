...
def parse_fastqc(fastqc_path: pathlib.Path, sample_path: pathlib.Path, prefix="fastqc"):
    """
    ...
    for file in fastqc_path.iterdir():
        if "reads" in file.name and "." not in file.name:
            shutil.move(
                fastqc_path / file.name / "fastqc_data.txt",
                sample_path / f"{prefix}{file.suffix}.txt"
            )
    ...
    # Parse data file(s)
    for suffix in [1, 2]:
        path = sample_path / f"{prefix}{suffix}.txt"
        ...
        for line in handle:
            ...
            # The statements below handle the parsing of lines when the flag has been set or a multi-line
            # section. This ends when the 'END_MODULE' line is encountered and the flag is reset to None
                ...
                # Convert position field to a one or two-member tuple
            ...
            elif flag == "sequence" and "#" not in line:
...
def average_list(list1, list2):
    if not isinstance(list, list) or not isinstance(list2, list):
